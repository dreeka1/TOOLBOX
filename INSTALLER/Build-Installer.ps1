...
$portablePath = Join-Path $projectRoot 'TOOLBOXV6.6.exe'
$scriptPath = Join-Path $PSScriptRoot 'TOOLBOX_v6.6.nsi'
$outputPath = Join-Path $projectRoot 'output\installer\CONTPAQi_Toolbox_Setup_v6.6.exe'
$compilerCandidates = @(
...
if ((Get-Item -LiteralPath $portablePath).VersionInfo.FileVersion -ne '6.6.0.0') {
    throw 'El ejecutable portable no corresponde a la version 6.6.0.0.'
}
...
if ($output.VersionInfo.FileVersion -ne '6.6.0.0') { throw 'La version del instalador generado no es 6.6.0.0.' }
